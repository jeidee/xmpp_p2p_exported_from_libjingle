...
public:
	CustomXmppPump() : XmppPump(this), server_(false), file_(new talk_base::FileStream) {
	}
	...
	bool ProcessStream(talk_base::StreamInterface* stream,
		...
		std::cout << stream->GetState() << std::endl;
...
int main(int argc, char **argv) {
	...
	cricket::SimplePortAllocator allocator(&network_manager, "pcp_agent");

	//std::vector<std::string> relay_hosts;
	//std::vector<talk_base::SocketAddress> stun_hosts;
	//relay_hosts.push_back("10.5.23.33");
	//stun_hosts.push_back(talk_base::SocketAddress("msg.iam0.com", 3478));
